...
function buildCoverPage() {
  return [
    ...
    new Paragraph({
      ...
      spacing: { before: 80, after: 60 },
      children: [
        new TextRun({ text: 'Ngày báo cáo: 31/08/2026', font: FONT, size: 22, bold: true }),
      ],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      ...
      children: [
        new TextRun({
          text: 'Repository: https://github.com/toanteng11/Microlearning-RestFullAPI-FullStack-App',
          font: FONT,
          size: 20,
        }),
      ],
...
function buildIntro() {
  return [
    ...
    p([
      ...
      t(
        ' đã hoàn thành, bao gồm: infrastructure as code, container hóa ứng dụng, CI/CD pipeline, Staging deployment, cloud smoke/E2E và toàn bộ bộ evidence. Đồng thời trình bày kế hoạch triển khai ',
      ),
    ...
    p([
      t(
        'Tên công nghệ, actor, capability, status và contract được giữ bằng tiếng Anh để đồng nhất với source code; phần diễn giải được trình bày bằng tiếng Việt để thuận tiện cho việc đọc, review và nghiệm thu.',
      ),
    ]),
    spacer(),
  ];
...
function buildChapter1() {
  const progressRows = [
    [
      'Business Analysis baseline',
      'COMPLETED',
      '22 nhóm tài liệu BA đã liên kết và kiểm soát thay đổi.',
    ],
    [
      'Phase 01 - Project Foundation',
      'COMPLETED',
      'Monorepo, Docker, Swagger, CI và quality gates.',
    ],
    [
      'Phase 02 - Authentication And User Management',
      'COMPLETED',
      'JWT/refresh rotation, invitation, RBAC và user governance.',
    ],
    [
      'Phase 03 - Classroom Foundation',
      'COMPLETED',
      'Classroom, class code/link enrollment, ownership và membership.',
    ],
    [
      'Phase 04 - Learning Content',
      'COMPLETED',
      'Course, Module, Lesson, Flashcard, Deadline, Announcement và To-do.',
    ],
    [
      'Phase 05 - Assessments And Grading',
      'COMPLETED',
      'Quiz, Assignment, Attempt, Submission, Grade/Regrade và deadline exception.',
    ],
    [
      'Phase 06 - Reporting And Analytics',
      'COMPLETED',
      '68/68 Must Pass; reporting, Gradebook, privacy, read model và analytics.',
    ],
    [
      'Phase 07 - DevOps And Deployment',
      'COMPLETED',
      '66/66 Must Pass; container, IaC, WIF, CD pipeline, staging deploy, E2E Pass.',
    ],
    [
      'Phase 08 - Production Release',
      'TIẾP THEO',
      'Production Atlas, custom domain, UAT, Go/No-Go và Production apply.',
    ],
  ];
  ...
  const archRows = [
    [
      ...
      'Giao diện Admin, Teacher, Student và gọi RESTful API.',
    ],
    [
      'Backend',
      'Node.js, Express, TypeScript',
      'Business rules, services, validation, authorization và API.',
    ],
    ...
    [
      ...
      'Production image, smoke, Trivy scan, CycloneDX SBOM.',
    ],
    [
      'IaC',
      'Terraform, GCS remote state',
      'Cloud Run, Secret Manager, Artifact Registry, Monitoring.',
    ],
    [
      'CI/CD',
      'GitHub Actions, WIF/OIDC',
      'Lint, test, build, publish, deploy, smoke, E2E và security gates.',
    ],
    [
      'Cloud target',
      'Google Cloud Run (asia-southeast1)',
      'Staging COMPLETED; Production deploy thuộc Phase 08.',
    ],
  ];
  ...
  return [
    ...
    p([
      t(
        'Microlearning Classroom LMS Platform là hệ thống LMS nội bộ hỗ trợ giảng dạy theo mô hình microlearning. Sản phẩm tham khảo nghiệp vụ phổ biến của Google Classroom nhưng được phân tích và triển khai độc lập, tập trung vào nội dung ngắn, deadline, đánh giá, phản hồi và theo dõi tiến độ.',
      ),
    ]),
    p([
      t(
        'Mục tiêu hiện tại là hoàn thiện một sản phẩm Full Stack có RESTful API rõ ràng, giao diện React theo actor, dữ liệu MongoDB có integrity, Swagger/OpenAPI đồng bộ và quy trình DevOps có thể tái lập từ local đến Google Cloud.',
      ),
    ]),
    ...
    p([
      t('Admin'),
      t(
        ' quản trị Student, Teacher, Admin, invitation và governance metadata trong phạm vi được phép.',
        {},
      ),
    ]),
    p([
      t('Teacher'),
      t(
        ' sở hữu Classroom/Course, xây dựng nội dung, tạo assessment, chấm điểm và theo dõi tiến độ bằng dashboard, ranking và Gradebook.',
        {},
      ),
    ]),
    p([
      t('Student'),
      t(
        ' tự đăng ký, đăng nhập trước khi join bằng class code hoặc invitation link, học và theo dõi To-do, progress cùng Grade đã được trả.',
        {},
      ),
    ]),
    p([
      t('Super Admin'),
      t(
        ' có quyền governance mở rộng nhưng vẫn tuân thủ audit, privacy projection và không đọc dữ liệu học tập riêng tư ngoài contract.',
        {},
      ),
    ]),

    heading2('1.3. Kiến trúc và công nghệ hiện hành'),
    simpleTable(['Lớp hệ thống', 'Công nghệ/giải pháp', 'Vai trò'], archRows),
    ...
    p([
      ...
      t(
        ' đã hoàn thành toàn bộ pipeline: CI → Build & Publish → Deploy Staging → Cloud Smoke & E2E, với stable deployment record ',
      ),
    ...
    simpleTable(
      ...
      progressRows.map((row) => [
        row[0],
        {
          text: row[1],
          bold: true,
          color: row[1] === 'COMPLETED' ? C.GREEN : row[1] === 'TIẾP THEO' ? C.ORANGE : C.BLACK,
        },
        row[2],
      ]),
    ),
...
function buildChapter2() {
  ...
  return [
    ...
    p([
      ...
      t(
        'React, API, Swagger UI và health/version chạy cùng origin theo route contract. SPA deep link, API 404 và asset 404 đều được xác minh.',
      ),
    ]),
    p([
      ...
      t('node', { bold: true }),
      t(
        '; không chứa secret hoặc dev artifact; image size 198MB; SIGTERM graceful shutdown trong budget.',
      ),
    ]),
    p([
      ...
      t(
        ' (CVE-2026-14456 đã được patch bằng Node 24.20.0); CycloneDX SBOM 167 components; production container CI Pass 1m50s.',
      ),
    ]),
    p([
      t('Environment validation: ', { bold: true }),
      t(
        'Production config fail-fast với missing/insecure values; version endpoint phản ánh exact commit/digest/build/environment.',
      ),
    ]),
    ...
    p([
      ...
      t(
        'GCS remote state private, versioned, tách environment (staging/production). Không có secret trong state hoặc plan.',
      ),
    ]),
    p([
      t('Modules: ', { bold: true }),
      t(
        'cloud-run-service, cloud-run-seed-job, monitoring, secret-containers, artifact_registry. Mỗi module có README và outputs.',
      ),
    ]),
    p([
    ...
    p([
      ...
      t(
        'check-terraform-plan.mjs enforce không có unexpected destroy, public IAM hoặc cross-environment mutation. Policy SHA256 ',
      ),
    ...
    p([
      ...
      t(
        'GitHub Actions xác thực Google Cloud bằng OIDC/WIF, lock đúng repository, branch và environment. Không có service-account JSON key. Runtime, deploy và seed identities tách biệt theo least privilege.',
      ),
    ]),
    p([
      t('Secret Manager: ', { bold: true }),
      t(
        'Secret versions confirmed enabled: mongodbUri:6, accessToken:1, authIdentityPepper:1, classroomCodePepper:1, seedDemoPassword:1. Không có secret value trong Git, log hoặc Terraform state.',
      ),
    ]),
    p([
      t('MongoDB Atlas Staging: ', { bold: true }),
      t(
        'Database/user riêng, TLS, bounded pool (max 10), timeouts, synthetic-only data. Network waiver có expiry ',
      ),
    ...
    p([
      ...
      t(
        'readiness, liveness, release-identity (version/commit/digest/environment match), single-origin-web, api-documentation.',
      ),
    ]),
    p([
      t('Cloud security report (11/11 PASS): ', { bold: true }),
      t(
        'health-readiness, HSTS, Content-Security-Policy, nosniff, referrer-policy, release-identity, SPA-routing, Swagger/OpenAPI, not-found-routing, CORS, proxy-rate-limit.',
      ),
    ]),
    p([t('Cloud E2E – 4/4 roles PASS: ', { bold: true })]),
    p([t('• Student: reviews learning work và keeps a secure concurrent session.')], {
      indent: 0.3,
    }),
    p([t('• Teacher: reviews owned progress và Gradebook; foreign ownership stays hidden.')], {
      indent: 0.3,
    }),
    ...
    p([
      ...
      t(
        'Cloud Monitoring được apply qua Terraform trong Deploy Staging run. Uptime check targeting /ready endpoint. Structured logging với request/revision/commit/digest metadata.',
      ),
    ]),
    p([
    ...
    p([
      ...
      t(
        'decision: PASS, stable: true, stableAt: 2026-08-31T05:49:29.054Z. Ghi đầy đủ commit, digest, revision, secret versions, smoke/security/role report SHA256.',
      ),
    ]),
    spacer(),
  ];
...
function buildChapter3() {
  ...
  const ciRows = [
    [
      'PR #31 CI (release PR)',
      '6/6 checks Pass',
      'Production container, E2E, MongoDB, OpenAPI, Audit, Secret scan',
    ],
  ...
  return [
    ...
    p([
      ...
      t(
        '0 Critical, 0 High findings. CVE-2026-14456 đã được patch bằng Node 24.20.0-alpine (PR #31).',
      ),
    ]),
    p([
    ...
    p([
      ...
      t(
        '0 service-account JSON key. GitHub OIDC → WIF → short-lived token. Non-main PR không được cấp deploy credential.',
      ),
    ]),
    p([
      t('SBOM: ', { bold: true }),
      t(
        'CycloneDX SBOM 167 components, SHA256 c31ad88a889ae316bd0bf014f2a2b349563ef7e4966e79443b4d80f96a9cd941.',
      ),
    ]),
    ...
    p([
      ...
      t(
        'Deploy Staging checkout exact release commit; npm ci từ clean state; build artifacts và smoke Pass tại CI runner.',
      ),
    ]),
    ...
    simpleTable(
      ...
      [
        [
          'Atlas network waiver hết hạn',
          'Mất kết nối Staging',
          'Expires 2026-09-13; cần renew trước Phase 08',
        ],
        [
          'Production Atlas Free tier',
          'Không có backup/private connectivity',
          'Blocked cho Phase 08 – mua paid tier',
        ],
...
function buildChapter4() {
  const phase8Rows = [
    [
      '1. Production Atlas',
      'Nâng Atlas Free lên paid M10+; bật private connectivity và native backup.',
      'NO-GO nếu chưa xong',
    ],
    [
      '2. Custom domain/TLS',
      'Mua/cấu hình domain; map Cloud Run custom domain; verify HTTPS certificate.',
      'Recommended',
    ],
    [
      '3. Production Terraform',
      'Áp dụng Terraform module production environment; plan review và apply.',
      'Required',
    ],
    [
      '4. Static egress/NAT',
      'Cấu hình Cloud NAT + static IP cho Atlas IP allowlist production.',
      'APPROVED_NA nếu waiver',
    ],
    [
      '5. UAT & System Test',
      'Full E2E với dữ liệu UAT; sign-off Product Owner và Technical Lead.',
      'Required',
    ],
    [
      '6. Go/No-Go gate',
      'Review: 66 AC, security, cost, backup, UAT, handoff accepted.',
      'Required',
    ],
    [
      '7. Production deploy',
      'Promote exact Staging digest lên Production Cloud Run.',
      'Sau Go/No-Go',
    ],
    [
      '8. Smoke & monitor',
      'Production smoke, alert test, uptime check và dashboard verify.',
      'Required',
    ],
    [
      '9. Handoff & closure',
      'Final evidence register, exit report, release notes và archive.',
      'Final gate',
    ],
  ];
  ...
  return [
    ...
    p([
      t(
        'Phase 07 đã chuẩn bị đầy đủ readiness cho Production release: exact image digest traceability, WIF identity pipeline, Terraform module production-ready, deployment record contract, smoke/security/E2E evidence đầy đủ và rollback path đã được rehearsal.',
      ),
    ]),
    p([
      ...
      t(
        ' làm điểm bắt đầu. Tất cả Must AC Phase 07 đã Pass; Phase 08 chỉ cần thực hiện Production apply sau khi Go/No-Go được duyệt.',
      ),
    ]),
    ...
    p([
      t('Build once, promote by exact digest: ', { bold: true }),
      t(
        'Phase 08 promote đúng image digest đã được scan/smoke/E2E xác nhận ở Phase 07. Không rebuild lại image.',
      ),
    ]),
    p([
      t('Production apply chỉ sau Go/No-Go: ', { bold: true }),
      t('Protected workflow; không deploy Production sớm để làm đẹp báo cáo tiến độ.'),
    ]),
    p([
      t('UAT sign-off bắt buộc: ', { bold: true }),
      t('Product Owner và Technical Lead phải sign-off trước Go/No-Go.'),
    ]),
    p([
      t('Atlas paid tier là blockers: ', { bold: true }),
      t('Không có Production backup và private connectivity = NO-GO.'),
    ]),
    p([
      t('Rollback plan: ', { bold: true }),
      t('Prior stable digest đã được ghi trong deployment record; rollback < 60s theo runbook.'),
    ]),
...
function buildChapter5() {
  const doneRows = [
    [
      ...
      'Phase 07 - Done',
      'Phase 08 - Must repeat for Production',
    ],
    ...
    [
      'Exact commit/digest/revision/URL evidence',
      'Done (stable record)',
  ...
  return [
    ...
    simpleTable(
      ...
      [
        ...
        [
          'Custom domain/DNS config sai',
          'HTTPS không hoạt động',
          'Test certificate + smoke trước go-live',
        ],
        [
          'UAT phát hiện bug nghiêm trọng',
          'Delay release',
          'Fix + re-test trong sprint; không skip',
        ],
        [
          'Terraform Production plan sai scope',
          'Destroy/public IAM',
          'Review plan kỹ; dùng protected workflow',
        ],
        [
          'Cost vượt budget trial',
          'Project bị suspend',
          'Monitoring billing alert + quota guardrails',
        ],
        [
          'Secret hoặc credential bị lộ',
          'Vi phạm bảo mật',
          'WIF, redaction gate, Gitleaks, rotate ngay',
        ],
      ],
    ),
    ...
    p([
      t('Promote exact digest Phase 07 đã Pass smoke/E2E; không rebuild lại image cho Production.'),
    ]),
    p([
      t(
        'Không paste credential vào Git, log, command history, screenshot hoặc Terraform variable/state.',
      ),
    ]),
    p([
      t(
        'Cloud apply chỉ chạy sau review Terraform plan; Production apply luôn bị chặn đến Go/No-Go.',
      ),
    ]),
    p([
      t(
        'Mọi UI/API/Cloud change phải có automated test, smoke, evidence và rollback consideration tương ứng.',
      ),
    ]),
    spacer(),
  ];
...
function buildConclusion() {
  return [
    ...
    p([
      ...
      t(
        ', 6 Conditional APPROVED_NA, 0 Critical/High defect. Toàn bộ CD pipeline từ main branch đến Staging cloud đã Pass: CI (7/7) → Build & Publish → Deploy Staging → Cloud Smoke & E2E (4/4 roles). Stable deployment record ',
      ),
      t('decision: PASS', { bold: true }),
      t(' tại revision '),
      t('microlearning-staging-00009-6bs', { bold: true }),
...
function buildAppendix() {
  const evidenceRows = [
    ...
    [
      '14',
      'Stable deployment record',
      'decision: PASS; stable: true; stableAt: 2026-08-31T05:49:29Z',
    ],
  ...
  return [
    ...
    p([
      ...
      t(
        'Báo cáo phản ánh Phase 07 đã COMPLETED với stable deployment record tại checkpoint 31/08/2026. Báo cáo không tuyên bố Production đã được deploy trước khi có Go/No-Go Phase 08.',
      ),
    ]),
  ];
}
...
async function buildDocument() {
  ...
  const outPath =
    'artifacts/progress-report/bao-cao-tien-do-microlearning-classroom-lms-phase7-va-chuan-bi-phase8.docx';
