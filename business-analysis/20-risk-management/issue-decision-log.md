...
| --- | --- | --- | --- | --- | --- |
| ISS-001 | Cloud provider, region, account/service, quota/cost owner và production-like environment chưa được chọn ở BA stage; điều này chặn Cloud readiness cụ thể. | R-003, R-020, TR-GAP-006 | DevOps + Product Owner | Đề xuất option, ghi ADR, xác nhận account/quota/secret/monitoring/deployment direction. | Before Staging Cloud / Open |
| ISS-002 | CI/CD implementation, required checks, artifact registry/provenance và actual pipeline evidence chưa tồn tại vì code/pipeline chưa được triển khai. | R-012, R-023, TR-GAP-004 | DevOps + Technical Lead + QA Lead | Thiết kế/implement pipeline, config gate và link build/test/deploy evidence trước RC. | Before Release Candidate / Open |
...
| DEC-010 | Token nhạy cảm như invitation token không lưu raw trong database; raw link chỉ trả cho Admin có quyền ở thời điểm tạo/copy. | Giảm security exposure và đáp ứng token lifecycle/audit. | Technical Lead + Security Reviewer | Recorded Baseline (Draft) | NFR-SEC-003/005, BR-044/049 |
| DEC-011 | Cloud provider chưa bị khóa ở BA stage; lựa chọn provider/region/service/secret/monitoring phải được ghi ADR trước Cloud deployment. | Tránh claim triển khai cụ thể khi chưa có account/budget/capability decision. | DevOps + Product Owner | Recorded Baseline (Draft) | ASM-014, TR-GAP-006, ISS-001 |
