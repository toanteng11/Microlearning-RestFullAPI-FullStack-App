...
| AC ID | Given / When | Then | Priority | Traceability |
| --- | --- | --- | --- | --- |
| AC-INV-001 | Given Admin authorized nhập email Teacher hợp lệ, When tạo invitation | Then invitation PENDING/MANUAL_COPY có expiry; raw link chỉ có trong one-time create response để copy, list/detail/copy-event không tái tạo link và database chỉ lưu token hash. | Must | FR-006, BR-011 đến BR-014C, BR-042 đến BR-044 |
...
| AC-SEC-001 | Given password/token/invitation/upload/export action, When inspected through API/log/database/UI | Then plain password/raw token/secret không bị lộ ngoài approved one-time credential response/request memory; list/detail/log/audit/database tuân theo hash/redaction/access policy. | Must | NFR-SEC/PRV, BR-044, BR-052, BR-102 |
