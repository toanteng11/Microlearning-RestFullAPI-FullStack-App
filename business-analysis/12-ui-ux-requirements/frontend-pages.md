...
| UI-005 | `/teacher/invite?token=:token` | Invited Teacher | Teacher mở link mời; React capture rồi xóa query token trước khi preview/activation. | `POST /api/v1/teacher/invitations/preview`, `POST /api/v1/teacher/invitations/accept` | Loading, token expired, validation error, success | P0 |
...
| UI-007 | `/join/invite#token=:token` | Guest/Student | React capture/xóa fragment, preview Classroom và revalidate trước khi join sau Login. | `POST /api/v1/classrooms/invite-links/preview`, `POST /api/v1/classrooms/join-by-token` | Loading, token invalid, login required, wrong role, already joined, success | P0 |
...
| UI-027 | `/teacher/classrooms/:classroomId/settings` | Teacher | Cập nhật Classroom settings, Class Code và Invite Link. | `PATCH /api/v1/classrooms/{classroomId}/settings`, `POST /api/v1/classrooms/{classroomId}/class-code/regenerate`, `POST /api/v1/classrooms/{classroomId}/invite-links`, `POST /api/v1/classrooms/{classroomId}/invite-links/{linkId}/regenerate`, `POST /api/v1/classrooms/{classroomId}/invite-links/{linkId}/disable` | Loading, validation, stale conflict, one-time copy, confirm, success | P0 |
