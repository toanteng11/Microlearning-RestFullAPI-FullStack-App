...
| ADR-010 | Managed Cloud Services Direction | Proposed | DevOps / Product Owner | Ưu tiên managed runtime/database/storage cho Staging/Production, vendor/region cần chốt. |
...
| Decision | Docker image cho frontend/backend, Docker Compose cho Local; config và secret inject runtime, không hard-code/bake vào image. |
| Consequences | Reproducible build/deploy; team cần duy trì Dockerfile, image scan, env inventory và volume/network local. |
| Validation | Local compose, CI build, staging deploy, secret scan/config review. |

### ADR-010: Managed Cloud Services Direction

| Field | Nội dung |
| --- | --- |
| Status | Proposed |
| Context | Đồ án cần học DevOps/Cloud nhưng team không nên chịu vận hành database/storage cluster phức tạp khi chưa cần. |
| Options considered | Managed service; self-managed VM/container; Kubernetes ngay từ đầu. |
| Decision | Hướng Production/Staging ưu tiên managed static hosting/CDN, managed container/VM runtime, managed MongoDB và managed object storage; provider/region/cost cần chốt. |
| Consequences | Giảm vận hành nền tảng, có backup/monitoring integration; phụ thuộc provider/cost/network model. |
| Validation | Environment matrix, cost estimate, security/backup/rollback rehearsal. |
