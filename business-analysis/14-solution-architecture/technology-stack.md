...
| Database | MongoDB / MongoDB Atlas | Baseline | MongoDB cho Local/CI; MongoDB Atlas là managed database target cho Cloud. |
...
| CI/CD | GitHub Actions | Accepted baseline | Lint, test, build, scan, publish image, deploy Staging/Production, smoke test và approval gate. |
| Cloud runtime | Google Cloud Run | Accepted baseline | Một application container phục vụ React static bundle, REST API và Swagger cùng origin; scale-to-zero cho tải nhỏ. |
| Container registry | Google Artifact Registry | Accepted supporting service | Lưu image theo immutable commit tag/digest và hỗ trợ promotion/rollback. |
| Cloud secret | Google Secret Manager | Accepted supporting service | Cấp `MONGODB_URI`, JWT/session secret và protected runtime value cho Cloud Run. |
| Cloud observability | Cloud Logging và Cloud Monitoring | Accepted supporting service | Central log, metric, dashboard, alert và deployment diagnosis. |
...
| Static assets | React build được đóng vào Cloud Run production image | Giữ Web/API cùng origin cho refresh cookie `SameSite=Lax`, không phụ thuộc Firebase | Tách Web/API qua hai site mà không review lại cookie/CORS/security |
...
| Production Atlas tier | Chọn Free/Flex/M10 theo capacity, backup, private network và RPO/RTO | Product Owner + DevOps + Backend Lead | Trước dữ liệu Production thật |
| Domain/TLS | Dùng mặc định `run.app` hay custom domain; DNS ownership và renewal | Product Owner + DevOps | Trước public Production release |
...
| Observability policy | Alert thresholds, retention, dashboard và escalation owner trên Cloud Logging/Monitoring | DevOps | Trước release Staging |
