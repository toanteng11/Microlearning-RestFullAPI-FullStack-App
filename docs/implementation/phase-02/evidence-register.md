...
| P02-EVD-DATA-002 | Replica set transaction behavior                                | Mongo/Compose/CI           | Local Pass; Remote Pending | `mongodb-transaction.integration.test.ts`: commit + rollback `2/2`; `rs0` primary (`2026-07-17`) |
...
| P02-EVD-OPS-001  | Docker build/non-root/Compose health                            | Command record             | Local Pass; Remote Pending | Web/API/Mongo healthy; init exit `0`; `/ready` Mongo `UP`; API uid `1000(node)` (`2026-07-17`)   |
...
| P02-EVD-OPS-005  | Typed auth environment fail-fast boundaries                     | Vitest/config review       | Local Pass; Remote Pending | `environment.test.ts`: `10/10`; secret/origin/range/Production guards (`2026-07-17`)             |
