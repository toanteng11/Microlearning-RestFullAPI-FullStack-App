...
6. Chỉ sau đó tạo branch `feature/phase-05-assessments-grading`.
...
```powershell
git switch -c feature/phase-05-assessments-grading
git push -u origin feature/phase-05-assessments-grading
```

Không dùng tiền tố `codex/`. Mỗi PR nhỏ có thể dùng branch slice được liệt kê trong `phase-plan.md`, nhưng phải tách từ `main` mới nhất và không duplicate thay đổi đang chờ merge.
...
8. `testing-strategy.md`, `acceptance-criteria.md`, WBS/checklist/traceability.
...
- Per-Student deadline set/reset/history.
...
7. Frontend query cache/navigation state.
